...
struct AutoTrackSetupContent: View {
    ...
    static let steps: [(title: String, detail: String)] = [
        ("1. Open the Shortcuts app", "It comes preinstalled on iOS. Go to the Automation tab and tap New (+)."),
        ("2. Choose 'Wallet'", "On iOS 26 it's called 'Wallet' (older iOS calls it 'Transaction'). Select the cards you want tracked."),
        ("3. Run Immediately", "Turn on 'Run Immediately' and turn off 'Notify When Run'. Tap Next, then pick 'New Blank Automation'."),
        ("4. Add action → search 'BudgetMe'", "Add 'Log a Transaction in BudgetMe' (the grid icon, not the card one)."),
        ("5. Set the field to 'Shortcut Input'", "Tap the action's single 'Transaction' field, choose 'Shortcut Input', then tap Done. That's it — BudgetMe reads the merchant and amount itself.")
    ]
